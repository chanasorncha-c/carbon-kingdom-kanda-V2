  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 rounded-blob px-4 py-6" style={SCENE_STYLE}>
      ...
      <div className="rounded-blob bg-white/70 p-3 shadow-soft">
        <p className="mb-1 font-heading text-xs text-slate-500">{t('level4.yourAnswer')}</p>
        <div className="flex min-h-[44px] flex-wrap items-center gap-2">
          {built.length === 0 && <span className="font-body text-sm text-slate-400">—</span>}
          {built.map((id, i) => (
        ...
        <div>
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {availableCards.map(card => (
              <button
                key={card.id}
                className="min-h-[44px] rounded-blob bg-lavender/50 px-4 py-2 font-heading text-slate-800 shadow-soft transition hover:scale-105"
      ...
      <AnimatePresence>
        {status !== 'idle' && (
          ...
          >
            ...
            {status === 'wrong' && (
              <p className="mt-1 font-heading text-sm text-mint-ink">
