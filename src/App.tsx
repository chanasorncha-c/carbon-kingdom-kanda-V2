      ...
      <main className="pb-16">
        ...
        {screen === 'home' && player && (
          <div className="mx-auto flex max-w-lg flex-col items-center gap-6 rounded-blob bg-white/80 p-8 text-center shadow-soft">
            ...
            <p className="font-body text-sm text-slate-500">{t('home.levelPicker')}</p>
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:justify-center">
              {LEVELS.map(level => (
                <button
                  key={level.id}
                  className={`min-h-[44px] rounded-full px-8 py-3 font-heading text-lg shadow-soft transition hover:scale-105 ${COLOR_CLASSES[level.color]}`}
                  onClick={() => {
