    ...
    <div className="mx-auto max-w-3xl space-y-4 rounded-blob px-4 py-6" style={SCENE_STYLE}>
      ...
      <div className="grid grid-cols-2 gap-3">
        {q.choices.map(choice => {
          ...
          // Text color is paired per-background so it always stays readable:
          // the wrong-chosen coral tile is dark enough that slate text on it
          // fails contrast, so it gets white text instead.
          let bg = 'bg-white/70'
          let text = 'text-slate-800'
          if (status !== 'idle' && isCorrectChoice) {
            bg = 'bg-mint/85'
            text = 'text-slate-800'
          } else if (status === 'wrong' && isChosen) {
            bg = 'bg-coral/75'
            text = 'text-[#fff]'
          }
          return (
            <button
              key={choice.id}
              className={`min-h-[44px] rounded-blob px-4 py-3 font-heading shadow-soft transition hover:scale-105 ${bg} ${text}`}
